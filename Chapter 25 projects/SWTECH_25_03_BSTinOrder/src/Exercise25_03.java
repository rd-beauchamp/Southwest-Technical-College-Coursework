...
public class Exercise25_03 {
  public static void main(String[] args) {
    ...
    for (int i = 0; i < 10; i++) {
    	int sharedRock = input.nextInt();
     if(tree.insert(sharedRock)) tree.stackAdd(sharedRock);
    }
    
    System.out.println("InOrder using a recursive method: ");
    tree.inorder();
...
//    tree.nonRecursiveInorder();
  ...
  public static class BST<E extends Comparable<E>> extends AbstractTree<E> {
    ...
    protected Stack<E> stacktistics = new Stack<>();
    ...
    protected Stack<E> inorderWithStack(TreeNode<E> root){
    	Stack<E> sortedStack = new Stack<>();
    	
    	while(!stacktistics.isEmpty()) {
    		E temp = stacktistics.pop();
    	
    		while(!sortedStack.isEmpty() && sortedStack.peek().compareTo(temp) != -1) {
    			stacktistics.push(sortedStack.pop());
    		}
